...
case class RequestOpinion(belief: Int)
...
class Agent(stopThreshold: Double, distribution: Distribution) extends Actor {
    ...
    def receive: Receive = {
        ...
        case RequestOpinion(senderBelief) =>
            ...
            else if (senderBelief == belief) agentSender ! SendOpinion(1)
        ...
        case UpdateConfidence =>
            ...
            calculateOpinionClimate { climate =>
                ...
                if (confidence >= beliefExpressionThreshold & belief == 0)
                    network ! ConfidenceUpdated(aboveThreshold, confidence, 0)
                else if (confidence >= beliefExpressionThreshold & belief == 1)
                    network ! ConfidenceUpdated(aboveThreshold, confidence, 1)
                else if (confidence < beliefExpressionThreshold & belief == 0)
    ...
    override def preStart(): Unit = {
        distribution match {
            case Uniform =>
                belief = if (Random.nextBoolean()) 1 else 0
